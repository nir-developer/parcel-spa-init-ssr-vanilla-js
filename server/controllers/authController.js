...
const createSendToken = (user, statusCode, res) => {
  ...
  console.log('INSIDE CREATE SEND TOKEN: THE JWT IN THE HTTP ADDED TO THE RES OBJECT: ')
...
exports.signup = async (req,res) =>{
  ...
  try   
  {
    ...
    let user = new User({
     ...
     photo: req.file ? `img/users/${req.file.filename}` : `img/users/default.jpg`
...
//     //STEP 1: (FOR CLIENTS OTHER THAN BROWSERS) Getting token from request.headers object and check if it's there(Express make it lower case!)
...
//         return res.status(400).json({status:'fail', message:'User recently chnaged password'})
//     }

//     req.user = currentUser; 

//     console.log('PROTECT BEFORE NEXT()  - THE req.user : ')
...
exports.isLoggedIn = async (req,res,next) =>{
  ...
  // 6677e817fa3d904741b4023a
     ...
     // const user = await User.findById(req.user.id).select('-password')
      if(!user) return res.status(401).json({status:'fail', message:'User not authorized'})
